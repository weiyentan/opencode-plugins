...
**Status:** ✅ Phase 0 (scaffolding), Phase 1 (client infrastructure), and Phase 2 core tools complete — 12 tools implemented covering project lookup, template detail, inventory detail, job lifecycle, environment debugging, and interactive configuration.

**Coverage:** 12 of 22 AWX operations planned for v1 (~55%), covering the 80% use case. Full tool-action mapping table documented in the PRD.
...
| Module | File | Purpose |
|--------|------|---------|
| **Plugin entry** | `src/index.ts` | Registers all AWX tools (list-templates, list-projects, list-jobs, launch-job, job-status, wait-job, get-job-events, sync-project, get-resource, debug-env, configure); wires HTTP client, metrics lifecycle (load/persist/dispose), and dispose hook for plugin shutdown |
