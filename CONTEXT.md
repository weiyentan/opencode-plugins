...
| **sql.js** | Pure JavaScript/WebAssembly SQLite driver used by the plugin (replaces native `better-sqlite3`). Loaded asynchronously via `initSqlJs()`. Database buffer is read from disk with `readFileSync`. Read-only enforcement is handled at the tool-query level (not database-level), since sql.js has no native read-only mode. No native compilation, node-gyp, or MSVC required — critical for Windows compatibility. |
