 ...
 * ## Key Transformations
 *
 * - **Capacity fields**: Maps `capacity`, `consumed_capacity`, and
 *   `remaining_capacity` directly from the API response.
 * - **Envelope**: Wraps output in `{ schema_version, resource_type, id, data }`.
 *
 * ## Usage
 *
 * ```ts
 * const response = await fetch(client, "GET", "/api/v2/instance_groups/3/");
...
export function mapInstanceGroup(raw: unknown): InstanceGroupDetailOutput {
  ...
  const data: InstanceGroupData = {
    ...
    capacity: ig.capacity ?? 0,
    consumed_capacity: ig.consumed_capacity ?? 0,
    remaining_capacity: ig.remaining_capacity ?? 0,
    created: ig.created ?? "",
