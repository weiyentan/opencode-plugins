/**
 * launch-workflow.ts — Thin proxy that passes raw extra_vars to the AWX workflow launch API.
 *
 * No transforms, no validation — just POST raw extra_vars to
 * /api/v2/workflow_job_templates/{id}/launch/ and return the raw AWX response body.
 */
 ...
 *
 * Passes raw extra_vars as-is to POST /api/v2/workflow_job_templates/{id}/launch/
 * and returns the parsed JSON response body from AWX verbatim.
 *
 * @param client     - The AWX HTTP client
 * @param templateId - The workflow job template ID to launch
...
export async function launchWorkflow(
  ...
  if (!response.ok) {
    ...
    throw new Error(`AWX workflow launch failed: HTTP ${response.status}: ${detail}`);
  }
