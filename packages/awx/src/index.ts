  ...
  switch (result.resource_type) {
    ...
    case "user": {
      ...
      return [
        ...
        `  Auditor:   ${d.is_system_auditor ? "yes" : "no"}`,
        `  Org:       ${d.organization_name}`,
      ].join("\n");
    }
    case "team": {
...
async function server(input: PluginInput): Promise<Hooks> {
  ...
  return {
    ...
    tool: {
      ...
      "awx-get-resource": tool({
        ...
        args: {
          type: z
            ...
            .describe("Resource type to fetch. Supports: template, project, inventory, user, team, schedule, notification_template"),
          id: z
            .number()
            .int()
            .positive()
       ...
       *
       * Creates a user in AWX via POST /api/v2/users/.
