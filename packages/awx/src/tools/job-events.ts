/**
 * job-events.ts — Job events tool factory.
 *
 * awx-get-job-events: Retrieves job events from the AWX API at
...
export function createJobEventsTool(getAwxClient: () => Promise<AwxClient>) {
  return tool({
    description: [
