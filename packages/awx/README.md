...
The output contract is validated against fixture snapshots generated by the Python script `awx_job_detail.py`. These snapshots are the ground truth — the TypeScript `JobDetailOutput` interface must match them field-for-field.

```bash
# Run contract compatibility tests
npx vitest run tests/contracts/contract.test.ts
```

### Regenerating Contract Snapshots

The contract snapshots are generated from raw AWX API fixture files by a Python script. If you update the fixture files or the `awx_job_detail.py` transformation logic, regenerate the snapshots:
...
This reads `tests/fixtures/awx_job_*.json` and writes the contract-compliant output to `tests/contracts/__snapshots__/awx_job_*.json`. The generated snapshots should be committed to version control — they are the canonical reference for the TypeScript types.

**Fixture files** (raw AWX API responses):
- `tests/fixtures/awx_job_success.json` — Successful job completion
- `tests/fixtures/awx_job_partial.json` — Job still running (partial data)
- `tests/fixtures/awx_job_failure.json` — Failed job with unreachable hosts

**Snapshot files** (contract-compliant output):
- `tests/contracts/__snapshots__/awx_job_success.json` — Ground truth for success
- `tests/contracts/__snapshots__/awx_job_partial.json` — Ground truth for running
- `tests/contracts/__snapshots__/awx_job_failure.json` — Ground truth for failure
## Hot-Reload
