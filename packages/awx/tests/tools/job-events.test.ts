/**
 * Job Events Tool Tests
 *
 * Tests for the awx-get-job-events tool: registration, basic fetch,
 * event filtering, pagination, and error handling.
 *
 * Tests the factory `createGetJobEventsTool` registered under
 * hooks.tool["awx-get-job-events"].
 */
...
describe("AWX Get Job Events Tool", () => {
  ...
  it("returns error metadata when API returns non-OK status", async () => {
    const mockClient = mockAwxClient();
    (mockClient.request as ReturnType<typeof vi.fn>).mockResolvedValue(
  ...
  it("returns error metadata when request throws (network failure)", async () => {
    const mockClient = mockAwxClient();
  ...
  it("returns abort message when signal is pre-aborted", async () => {
    const hooks = await createHooks(mockPluginInput());
